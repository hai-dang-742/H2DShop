...
public class CartDao extends BaseDao{
	...
	public int TotalQuanty(HashMap<Long, CartDto> cart) {
		...
		for(Map.Entry<Long, CartDto> itemCart : cart.entrySet()) {
			totalQuanty += itemCart.getValue().getQuanty();// lấy từng số lượng ra 
		}
		return totalQuanty;
	}
	...
	public double TotalPrice(HashMap<Long, CartDto> cart) {
		double totalPrice = 0;
		for(Map.Entry<Long, CartDto> itemCart : cart.entrySet()) {
			totalPrice += itemCart.getValue().getTotalPrice(); // Tính tổng toàn bộ sản phầm trong giỏ hàng 
		}
		return totalPrice;
	}
}
